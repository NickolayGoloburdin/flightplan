...
def intersection(neededinfo1, neededinfo2):
    ...
    if (neededinfo1[1] == 0 and neededinfo2[2] == 0):
        ...
        y = (neededinfo1[1]*neededinfo2[3] - neededinfo2[1]*neededinfo1[3]) / \
            (neededinfo2[1]*neededinfo1[2] - neededinfo1[1]*neededinfo2[2])
        x = (-1*y*neededinfo1[2] - neededinfo1[3]) / (neededinfo1[1])
        return [x, y]
...
def findFarthest(info, index1, index2):
    ...
    for k in range(len(newlistOfCoors[0]) - 1):
        ...
        if (k != index1 and k != index2):
            ...
            dist = abs(info[1]*newlistOfCoors[0][k] + info[2]*newlistOfCoors[1]
                       [k] + info[3]) / (math.sqrt(info[1]*info[1] + info[2]*info[2]))
    ...
    if event.key == 'enter':
        ...
        for k in range(len(listOfCoors)):
            if (math.sqrt((listOfCoors[k][0] - listOfCoors[(k+1) % len(listOfCoors)][0])**2 + (listOfCoors[k][1] - listOfCoors[(k+1) % len(listOfCoors)][1])**2) > maxxLen):
                ...
                maxxLen = math.sqrt((listOfCoors[k][0] - listOfCoors[(k+1) % len(listOfCoors)][0])**2 + (
                    listOfCoors[k][1] - listOfCoors[(k+1) % len(listOfCoors)][1])**2)
        ...
        if not isSlopeInf:
            ...
            slope = (float(listOfCoors[whichCorners[1]][1] - listOfCoors[whichCorners[0]][1])) / (
                listOfCoors[whichCorners[1]][0] - listOfCoors[whichCorners[0]][0])
            ax = slope
            by = -1
            c = -1.0*slope*listOfCoors[whichCorners[1]
                                       ][0] + listOfCoors[whichCorners[1]][1]
        ...
        for k in range(len(listOfCoors)):
            ...
            if ((k != whichCorners[0] and k != whichCorners[1])):
                ...
                dist = abs(
                    ax*listOfCoors[k][0] + by*listOfCoors[k][1] + c) / (math.sqrt(ax*ax + by*by))
                ...
                if (dist > maxxDist):
                    ...
                    farthestIndex = k
        stoppingC = -1.0*ax * \
            listOfCoors[farthestIndex][0] - by*listOfCoors[farthestIndex][1]
        ...
        if not isSlopeInf:
            ...
            if (reachPoint > whereWeare):
                ...
                while (reachPoint > whereWeare):
                    ...
                    InformationOfTheLongestLines.append(
                        [slope, ax, by, whereWeare])
                    whereWeare = whereWeare + toAdd
            else:
                whereWeare = whereWeare - toAdd
                while (reachPoint < whereWeare):
        ...
        for k in range(len(newlistOfCoors[0]) - 1):
            ...
            if (k != whichCorners[0]):
                ...
                if (isSlopeInfinity([newlistOfCoors[0][k], newlistOfCoors[1][k]],  [newlistOfCoors[0][(k+1) % ll],  newlistOfCoors[1][(k+1) % ll]]) == True):
                    ...
                    indexx = farthestForInf(
                        [newlistOfCoors[0][k], newlistOfCoors[1][k]], k)

                    if (newlistOfCoors[0][indexx] > newlistOfCoors[0][k]):

                        listOfWindows.append(
                            [-1, 1, 0, newlistOfCoors[0][k] + 1])
                        listOfBetween.append(
                            [-1, 1, 0, newlistOfCoors[0][k] + 0.9999999999])
                    else:
                        listOfWindows.append(
                            [-1, 1, 0, -1*newlistOfCoors[0][k] - 1])
                        listOfBetween.append(
                            [-1, 1, 0, newlistOfCoors[0][k] - 0.999999999])

                else:

                    neededInfo = slopeaxbyc([newlistOfCoors[0][k], newlistOfCoors[1][k]],  [
                                            newlistOfCoors[0][(k+1) % ll],  newlistOfCoors[1][(k+1) % ll]])
                    farthest = findFarthest(neededInfo, k, (k+1) % ll)
                    stopc = findStoppingc(
                        [newlistOfCoors[0][farthest], newlistOfCoors[1][farthest]], neededInfo)
        ...
        for k in range(len(listOfBetween)):
            ...
            insidePolygonCoors.append(intersection(
                listOfBetween[k], listOfBetween[(k+1) % lll]))
        ...
        def animation_frame(i):
            ...
            for lnum, line in enumerate(lines):
                ...
                line.set_data(listOfLines[lnum][0][:i],
                              listOfLines[lnum][1][:i])

            return lines

        animationn = FuncAnimation(fig, animation_frame, init_func=init, frames=np.arange(
            1, len(tpx), 1), interval=250, blit=True)
